...
ConstantFoldingPass::ConstantFoldingPass(const std::unordered_map<std::string, int64_t>& manifests)
    : manifests(manifests) {}
...
ExprPtr ConstantFoldingPass::visit(BinaryOp* node) {
    ...
    // Strength reduction and identity operations
    if (auto* right_num = dynamic_cast<NumberLiteral*>(right.get())) {
        ...
        // Strength reduction: divide by 2 -> right shift by 1
    ...
    if (auto* left_num = dynamic_cast<NumberLiteral*>(left.get())) {
        // Identity operations with left operand
...
ExprPtr ConstantFoldingPass::visit(ConditionalExpression* node) {
    ...
    // Constant folding: if condition is a constant, return appropriate branch
...
StmtPtr ConstantFoldingPass::visit(IfStatement* node) {
    ...
    // Constant folding: if condition is constant, eliminate the conditional
    if (auto* cond_lit = dynamic_cast<NumberLiteral*>(new_cond.get())) {
        if (cond_lit->value != 0) {
...
StmtPtr ConstantFoldingPass::visit(ForStatement* node) {
    // For ForStatement, we only do constant folding on the expressions, not LICM
    // LICM will be handled by LoopInvariantCodeMotionPass
